...
public class HiloJugador implements Runnable{
    ...
    public HiloJugador(Partida partida, Usuario usuario) throws IOException {
        this.partida = partida;
        this.usuario = usuario;
        if (partida == null) {
            this.partida = new Partida();
            this.utils.agregarPartida(partida);
        }
        //Metodo encargado de buscar el usuario correspondiente
        // this.buscarUsuario(utils.getListaUsuario());
        this.validarPartidaSinHilo();
    }
    ...
    private void validarPartidaSinHilo () {
        if (this.partida.getUsuario1() == null) {
            this.partida.setUsuario1(usuario);
            this.partida.setSiguienteJugador(usuario);
            usuario.setMarca('X');
    ...
    */
    ...
    public void buscarUsuario(List<Usuario> listaUsuario, String nombreUsuario) throws IOException {
        boolean agregarNuevoUsuario = true;
        while (true) {
            agregarNuevoUsuario = true;
            for (Usuario usuario: listaUsuario) {
